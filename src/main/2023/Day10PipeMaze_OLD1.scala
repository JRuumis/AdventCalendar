...
object Day10PipeMaze_OLD1 extends App {
    ...
    //val upLeftCorner = Coord(-0.5, -0.5)
    //val upRightCoord = Coord(0.5, -0.5)
    //val downLeftCoord = Coord(-0.5, 0.5)
    //val downRightCoord = Coord(0.5, 0.5)
    ...
    // Set - side A
    // Set - side B
    val twoSides2: Map[Char, (Set[Coord], Set[Coord])] = Map(
        'F' -> (Set(downLeftCoord, upLeftCoord, upRightCoord), Set(downRightCoord)),
        '7' -> (Set(upLeftCoord, upRightCoord, downRightCoord), Set(downLeftCoord)),
        'J' -> (Set(upRightCoord, downRightCoord, downLeftCoord), Set(upLeftCoord)),
        'L' -> (Set(upLeftCoord, downLeftCoord, downRightCoord), Set(upRightCoord)),
        '-' -> (Set(upLeftCoord, upRightCoord), Set(downLeftCoord, downRightCoord)),
        '|' -> (Set(upLeftCoord, downLeftCoord), Set(upRightCoord, downRightCoord))
    )
    ...
    val xxx = cellsForShoelace(updatedWalkedCells.toSet, newStartCell) :+ newStartCell
    ...
    val coordsForShoelace: Vector[Coord] = xxx.map(_.coord)
